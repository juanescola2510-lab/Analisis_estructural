...
# NUEVAS OPCIONES DE EDICIÓN DE PESO
...
# Tensión Total con los pesos de la barra lateral
t_total_lb = p_cad_lb + p_cang_lb + p_mat_lb + f_exc_lb
f_n = t_total_lb * 4.44822
reaccion_n = f_n / 2

# ÁREA DE LA SECCIÓN TRANSVERSAL (Área Simple)
...
with col2:
    ...
    ax2.axvline(0.27, color='orange', ls='-', lw=3)
    ax2.text(0.27, 0.8, "PLANO DE\nCORTE", color='orange', ha='center', fontweight='bold')
    ax2.axis('off'); st.pyplot(fig2)

# --- BLOQUE 2: COMPARATIVA DE FATIGA ---
st.markdown("---")
st.write("### Análisis de Fatiga (Área Simple)")
...
with col_graf:
    ...
    ax3.scatter(tau_m, tau_a, color='red', s=200, edgecolor='black', zorder=10, label='Operación')
...
with col_tab:
    ...
    st.info(f"Área Seccional Analizada: {area_pin:.2f} mm²")
    st.table({
        "Criterio": ["Soderberg", "Goodman", "Gerber"],
        "FS": [f"{fs_sod:.2f}", f"{fs_goo:.2f}", f"{fs_ger:.2f}"],
        "Estado": ["SEGURO" if fs_sod > 1.2 else "FALLA" for _ in range(3)]
    })
